...
model.add(Dense(512, input_dim=max_words, activation="relu"))   ###_YOUR_CODE_GOES_HERE_###) # Add first layer. Make sure to specify input shape
...
model.compile(optimizer="adam", loss="categorical_crossentropy", metrics=["acc"])     ###_YOUR_CODE_GOES_HERE_###)
...
model.fit(x=x_train, y=y_train, batch_size=batch_size, epochs=5, validation_data=(x_test, y_test))     ###_YOUR_CODE_GOES_HERE_###)
score = model.evaluate(x=x_test, y=y_test, batch_size=batch_size)   ###_YOUR_CODE_GOES_HERE_###)
...
part = "LqPRQ"
email = "bahadiryilmaz32@gmail.com"
secret = "hDLVEvAo0je4UTGk"
